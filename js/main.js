...
let list = document.getElementsByClassName("section__container__products__list")[0];
let copyProducts = JSON.parse(JSON.stringify(products));
let copyArr = JSON.parse(JSON.stringify(products));
copyArr.forEach( (item) => {
    if(typeof item.price === "string") {
        item.price = Number(item.price.replace(/\D+/g,""));
    } else {
        item.price = Number(item.price.newUan.replace(/\D+/g,""));
    }
} );
...
function showProducts (arr) {
    arr.forEach( item => {
        ...
        li.className = "section__container__products__list--item";
        let link = li.appendChild(document.createElement("a"));
        link.href = "";
        let wrapImg = link.appendChild(document.createElement("div"));
        wrapImg.className = "section__container__products__list--item__img";
        let img = wrapImg.appendChild(document.createElement("img"));
        img.src = "images/smartphone.svg";
        img.alt = "name";
        let description = link.appendChild(document.createElement("div"));
        description.className = "section__container__products__list--item__description";
        let name = description.appendChild(document.createElement("p"));
        name.className = "section__container__products__list--item__description--name";
        name.innerText = item.name;
        let descriptionWrap = description.appendChild(document.createElement("div"));
        descriptionWrap.className = "section__container__products__list--item__description--wrap";
        let price = descriptionWrap.appendChild(document.createElement("p"));
        price.className = "section__container__products__list--item__description--wrap--price";
        if( typeof item.price === "string") {
            price.innerText = "Цена: " + item.price;
        } else if (typeof item.price === "number") {
            price.innerText = "Цена: " + item.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ") + " грн";
        } else {
            price.innerText = "Цена: " + item.price.newUan
        }

        let reviews = descriptionWrap.appendChild(document.createElement("p"));
        reviews.className = "section__container__products__list--item__description--wrap--reviews";
        reviews.innerText = item.ratingRevievs;
    } )
}

function sortRating (arr, variable) {
    if(variable === "up") {
        arr.sort( (a,b) => {
            return a.ratingRevievs.split(" ")[0] - b.ratingRevievs.split(" ")[0]
        } );
    } else {
        arr.sort( (a,b) => {
            return b.ratingRevievs.split(" ")[0] - a.ratingRevievs.split(" ")[0]
        } );
    }
}

function sortPrice (variable) {
    if(variable === "up") {
        copyArr.sort( (a,b) => {
...
reviews.onchange = function () {
    ...
    if(this.value === "down") {
        list.textContent = "";
        sortRating(copyProducts, "down");
        showProducts(copyProducts);
    } else if (this.value === "up") {
        list.textContent = "";
        sortRating(copyProducts, "up");
        showProducts(copyProducts);
    } else {
        list.textContent = "";
...
price.onchange = function () {
    reviews.value = "default";
    if (this.value === "up") {
        list.textContent = "";
        sortPrice("up");
        showProducts(copyArr)
